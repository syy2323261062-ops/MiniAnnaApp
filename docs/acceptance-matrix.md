...
| Storage get/set | Versioned value under `mini-notes:notes:v1` via Anna Host API only | `src/anna/storage.ts`; `evidence/ui-no-llm-rpc.jsonl` | PASS |
| Empty input | `draft.trim()` guards button and submit handler | Real empty/whitespace/newline UI interaction | PASS |
| Save clears input | `setDraft("")` after successful Host API write | Real UI interaction | PASS |
| Ordered list | Persisted `nextOrder`; UI renders note order | Real UI interaction and sanitized storage values | PASS |
| Delete | Storage read, filtered value write, immediate render update | Sanitized UI RPC create/delete sequences | PASS |
| tools.invoke | Frontend resolves bundled handle/dev tool ID and invokes `summarize_notes` with note objects | `src/anna/tools.ts`; sanitized UI RPC log | PASS |
...
| `--no-llm` UI harness | Clean legacy-state UI session reaches `[-32603] harness started with --no-llm` and CRUD remains usable | `evidence/ui-no-llm-rpc.jsonl`; UI checklist | PASS |
